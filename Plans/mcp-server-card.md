...
- [ ] `https://easypay.thenextgen.store/.well-known/mcp/server-card.json` отдаёт HTTP 200 с `Content-Type: application/json` (или `application/json; charset=utf-8`).
- [ ] Тело ответа — валидный JSON, парсится `python -m json.tool` без ошибок.
- [ ] JSON содержит обязательные SEP-2127 поля: `$schema`, `name`, `version`, `description`. `name` — в формате `<reverse-dns>/<server>` с ровно одним слэшем.
- [ ] `remotes[0].url` равен `https://mcp.appload.tech/sse`, `type` равен `sse`.
- [ ] `https://easypay.thenextgen.store/robots.txt` остаётся `200 OK` и теперь содержит строку `Allow: /.well-known/`.
- [ ] `https://easypay.thenextgen.store/sitemap.xml` остаётся `200 OK` и содержит URL карточки в новом `<url>` блоке.
- [ ] `POST https://isitagentready.com/api/scan` с `{"url":"https://easypay.thenextgen.store"}` возвращает `checks.discovery.mcpServerCard.status == "pass"`.
- [ ] Существующие чеклисты из `DEPLOY.md` (hero, CTA, robots, sitemap, index.md) всё ещё зелёные после деплоя.
